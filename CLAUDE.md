...
- Cart page requires name, phone and delivery state/city before the WhatsApp
  send button enables.
- Work on and push to `main`. Every push to `main` deploys the static site to
  GitHub Pages (https://mmedaraj.github.io/ufon-weddings/) via
  `.github/workflows/pages.yml`; check the Actions run after pushing.
- The client reads static JSON (`client/public/data/*.json`, generated by
  `npm run export:data --prefix server`, git-ignored); the Nest API is optional.
  All asset paths go through `asset()` in `client/src/api.ts` so the site works
  under the `/ufon-weddings/` base path.
